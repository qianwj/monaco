...
import cn.elvis.monaco.core.transition.SubscribeTransition;
...
public class SubscribeHandler {
    ...
    private final SubscribeTransition transition = new SubscribeTransition();
    ...
    public Mono<Void> handle(String clientId, ClientPacket.Subscribe packet) {
        return dispatcher.dispatch(clientId, cid ->
                sessionStore.get(cid)
                        .flatMap(session -> {
                            var command = new Command.Subscribe(packet, cid, clock.now());
